...
p <- ggplot(data = top10, aes(collab_teachers, fct_reorder(name, collab_teachers), label = collab_teachers)) + 
  geom_point()
  
p1 <- p + geom_label() + theme(axis.text.y=element_text(angle=0,hjust = 1,size = 4)) +
  theme(axis.title = element_blank())
```

```{r}
score_sorted_1 <- agg %>% arrange(desc(effective_schlead))
top10_1 <- head(score_sorted_1, 10)

p <- ggplot(data = top10_1, aes(effective_schlead, fct_reorder(name, effective_schlead), label = effective_schlead)) +
  geom_point()
  
p2 <- p+geom_label() + theme(axis.text.y=element_text(angle=0,hjust = 1,size = 4)) +
  theme(axis.title = element_blank())
```

```{r}
score_sorted_1 <- agg %>% arrange(desc(trust))
top10_1<-head(score_sorted_1,10)

p <- ggplot(data = top10_1, aes(trust, fct_reorder(name, trust), label = trust)) +
  geom_point()
  
p3 <- p + geom_label() + theme(axis.text.y=element_text(angle=0,hjust = 1,size = 4)) +
  theme(axis.title = element_blank())
```

```{r}
score_sorted_1 <- agg %>% arrange(desc(rig_instruction))
top10_1<-head(score_sorted_1,10)

p <- ggplot(data = top10_1,aes(rig_instruction, fct_reorder(name, rig_instruction), label = rig_instruction)) + 
  geom_point()
  
p4 <- p + geom_label() + theme(axis.text.y=element_text(angle=0,hjust = 1,size = 4)) +
  theme(axis.title = element_blank())
```

```{r}
score_sorted_1 <- agg %>% arrange(desc(family_communityties))
top10_1 <- head(score_sorted_1,10)

p <- ggplot(data = top10_1, aes(family_communityties, fct_reorder(name, family_communityties), label = family_communityties)) +
  geom_point()
  
p5 <- p + geom_label() + theme(axis.text.y=element_text(angle=0,hjust = 1,size = 3)) +
  theme(axis.title = element_blank())
```
...
p <- ggplot(data = top10_1, aes(total_score, fct_reorder(name, total_score), label = total_score)) +
  geom_point()
  
p6 <- p + geom_label() + theme(axis.text.y=element_text(angle = 0,hjust = 1,size = 4))
```
...
As we can read from this graph, some schools, like Community School for Social Justice, HS Arts & Business YABC ranked high in several aspects, thus have a high comprehensive school. There are other schools, like Manhattan Center for Science and MAthematics, that ranked high only in particular aspects.
...
family_ <- family %>% mutate(t_s_disagree_percent = t_s_disagree/(t_s_disagree+t_s_agree+t_agree+t_disagree)) %>%
...
ggplot(data = data, aes(x = tq_score,y = perform_score)) +
  ...
  labs(title = "The relationship between teaching quality and students' performance")
```
...
tq_sel = tq_ %>% select(st_v_n,st_n,st_p,st_v_p,t_v_n,t_n,t_p,t_v_p,p_v_n,p_n,p_p,p_v_p)
